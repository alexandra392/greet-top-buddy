...
import { ArrowLeft, Calendar, Search, ExternalLink, FlaskConical, ShoppingBag, ChevronRight, TrendingUp, TextQuote } from "lucide-react";
    ...
    return (
      <div key={section.title} className="bg-muted/30 border border-border/40 rounded-xl p-4 flex flex-col">
        <div className="mb-3 pb-2 border-b border-border/40">
          ...
          <div className="grid grid-cols-[10px_1fr_56px_56px_14px] items-baseline gap-3 px-1.5 py-1.5 mt-2 rounded-md bg-muted/60 text-[8px] font-semibold uppercase tracking-widest text-muted-foreground">
            <span />
            <span>Categories ({totalCount})</span>
            <span className="text-right tabular-nums">{sectionTotal.toLocaleString()}</span>
                              ...
                              <tr key={inst.rank} className="border-b border-border/30 hover:bg-muted/20 transition-colors cursor-pointer" onClick={() => setSelectedInstitution(inst)}>
                                ...
                                <td className="text-right py-[3px]">
                                  <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-primary tabular-nums">
                                    ...
                                    <Asterisk className="w-2.5 h-2.5 opacity-60" strokeWidth={2} />
                                  </span>
                                </td>
                              </tr>
                            ))}
