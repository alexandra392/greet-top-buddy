...
const MarketActivity = () => {
  ...
  const SavedForTypeTable = ({ companyType }: { companyType: string }) => {
    const savedList = companies.filter(c => {
      ...
      if (companyType === 'projects') return c.entity_type === 'project';
    ...
    return (
      <div ref={savedTableRef} className="mt-2 mb-16 flex-shrink-0 border border-border/60 rounded-lg bg-card shadow-sm overflow-hidden">
            ...
            <TableBody>
              {savedList.map(company => (
                <TableRow key={company.id} className={`hover:bg-muted/20 border-b border-border/30 last:border-0 ${company.id === lastSavedId ? 'animate-saved-row-highlight' : ''}`}>
                  ...
                  <TableCell className="py-2 text-[10px] font-semibold text-foreground">{company.company_name}</TableCell>
                  <TableCell className="py-2 text-[10px] text-muted-foreground w-[120px]">{company.country}</TableCell>
                  <TableCell className="py-2 text-[10px] text-muted-foreground w-[100px]">
                    {companyType === 'projects' ? (company.scale === 'N/A' ? 'Unknown' : company.scale || 'Unknown') : getCompanySize(company.annual_revenue)}
                  </TableCell>
                  <TableCell className="py-2 text-[9px] text-muted-foreground w-[160px]">{activeTab === 'projects' ? company.sector : company.application}</TableCell>
                  <TableCell className="text-center py-2 w-[40px]">
